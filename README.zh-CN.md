...
| **`retrieval`** | 默认按兴趣查询 arXiv，也可选接入 OpenAlex 和 Semantic Scholar，在排序前统一去重，并写出候选 artifacts | 候选 JSON + 批次清单 |
...
2. **默认按这份画像去搜索 arXiv，也可扩展到 OpenAlex 和 Semantic Scholar**：不是根据你临时输入的关键词，而是基于你已经收集的论文证据
...
```bash
# 完整 digest：检查画像 → 文献检索 → 排序 → HTML 输出
...
1. profile_update    → 读取 Zotero 证据，构建紧凑研究地图
    ↓
2. retrieval         → 默认按兴趣检索 arXiv，也可选接入 OpenAlex 和 Semantic Scholar，统一去重后写出候选 JSON
    ↓
...
`research-assist` 默认只启用 **arXiv**。如果你想扩大候选池，可以任选以下源：

- `arxiv`
- `openalex`
- `semantic_scholar`

启用多源后，系统会先扩充候选池，再在排序前统一去重。

可以在 `config.json` 中这样设置：
...
- `arxiv` 仍是默认源，不需要额外凭证。
- `OpenAlex` 可匿名访问；`mailto` 和 `api_key` 都是可选项，但建议配置。
- `Semantic Scholar` 也可匿名访问；`api_key` 为可选项，可用于更高限额。
